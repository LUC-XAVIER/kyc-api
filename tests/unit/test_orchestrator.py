...
class _FakeStore:
    """Read-only DuplicateStore returning a canned duplicate outcome."""
...
def test_ocr_failure_rejects_before_any_face_work(wire) -> None:
    ...
    assert output.result.reject_reason == RejectReason.OCR_FAILED
...
def test_expired_id_is_rejected(wire) -> None:
    ...
    assert output.result.status is VerificationStatus.REJECTED
    assert output.result.reject_reason == RejectReason.ID_EXPIRED
...
def test_failed_liveness_skips_face_and_duplicate(wire) -> None:
    ...
    assert output.result.status is VerificationStatus.REJECTED
    assert output.result.reject_reason == RejectReason.LIVENESS_FAILED
...
def test_duplicate_goes_to_pending_without_enrollment(wire) -> None:
    ...
    assert output.embedding is None  # not enrolled on PENDING


def test_clean_pass_verifies_and_returns_embedding(wire) -> None:
    """A clean run verifies and hands back the selfie embedding to enroll."""
    store = wire(
        ocr=OcrResult(success=True),
        liveness=LivenessOutcome(passed=True, score=0.9, method="m"),
        face=FaceMatchOutcome(match_score=0.8, verified=True, threshold=0.4),
    ...
    assert output.result.status is VerificationStatus.VERIFIED
    assert output.result.confidence is not None
    assert output.embedding == _EMBEDDING
